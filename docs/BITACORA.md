...
## [2026-07-23] — La nota de continuación se refuerza con TIPOGRAFÍA, no con color: los colores que cantan son semántica
**Categoría:** visual
**Síntoma:** (decisión, no fallo) La nota de continuación salió en gris claro (`text-ink-faint`) y no se leía ("entre una mierda y nada", Antonio). El primer impulso fue "un color que cante".
**Qué se probó y DIO VERDE mientras el fallo estaba vivo:** ⭐ instrumento 5/5 y suite verde con la nota ilegible: ningún test mide "se lee". Lo cazó el ojo de Antonio.
**Causa raíz:** peso visual insuficiente. Pero "un color que cante" habría sido peor: los colores que cantan están TODOS ocupados por la semántica del cuadrante —rojo=imposible, naranja=incumplimiento, ámbar=aviso, verde=correcto— e índigo es la marca. Darle uno a la nota haría que el usuario viera algo naranja y entendiera "hay un problema aquí" cuando la nota solo dice "esto sigue el lunes": un canal semántico MINTIENDO un estado. La nota es información ESTRUCTURAL, no un estado.
**Cómo se cazó:** ojo humano (Antonio) sobre la página.
**Arreglo aplicado:** peso por TIPOGRAFÍA, canal libre: `font-bold` + `text-ink` (tinta oscura de composición). Medido el píxel real (Range del texto, no el bloque): "continúa el lunes" en negrita termina en 1325px, dentro del borde de rejilla (1350) — cabe en la columna del filo, 1 línea, sin truncar. Instrumento y suite verdes; 3.c ("cero texto en barra") intacto porque la nota es texto de la ficha, no de la barra.
**Ley que sale de aquí:** ⚠️ LA NOTA NO LLEVA COLOR QUE CANTE — su peso es TIPOGRÁFICO. Es la misma ley que dejó el botón Eliminar en tinta neutra: los colores del cuadrante son un vocabulario cerrado con significado, y prestárselos a información estructural es un canal mintiendo. Si dentro de seis meses alguien quiere ponerla naranja "para que se vea": no. El peso se sube con tamaño/grosor/tinta, no robando un estado. (Si la tipografía no bastara, el único color legítimo sería el índigo de marca —no es un estado—, y lo decide Antonio viendo.)
**Traza:** `src/components/FichaTurno.vue` (nota: font-bold text-ink).

## [2026-07-23] — El arrastre decidía retimar/reubicar por turno.dia: retimar la COLA de un partido lo mudaba de día en silencio
**Categoría:** rompe
**Síntoma:** (fallo previsto en diseño, evitado antes de existir) El arrastre elegía RETIMAR (misma celda) vs REUBICAR comparando la celda destino con `turno.dia`. Pero el trozo de COLA de un turno partido vive en `d+1`, no en `turno.dia`. Arrastrar esa cola dentro de SU PROPIA columna se habría leído como "otra celda" → REUBICAR: el usuario ajusta un poco la hora del trozo del viernes y, en vez de retimar, el turno se muda de día. Sin error, sin aviso, sin nada que chirríe.
**Qué se probó y DIO VERDE mientras el fallo estaba vivo:** NO APLICA — se detectó en el diseño de PC2 (punto 4.b), antes de escribir el código consciente del segmento. Nunca llegó a correr. Pero es exactamente el modo de fallo que el proyecto persigue: no rompe, HACE LO QUE NO ES.
**Causa raíz:** confundir el turno con su trozo. Un turno partido es UN ente con DOS trozos en días distintos; la decisión del arrastre es sobre el TROZO agarrado (su celda), no sobre el turno. Comparar con `turno.dia` mezcla los dos niveles.
**Cómo se cazó:** en el diseño, al pensar el arrastre sobre el modelo partido (yo mismo lo señalé como "un roce real que hay que resolver, no esconder").
**Arreglo aplicado:** `modoArrastre(destino, segmento, turno)` PURO compara el destino con la celda del SEGMENTO agarrado (`segmento.dia` + `turno.puesto`), no con `turno.dia`. El delta se aplica al inicio del TURNO ENTERO (ente único). El instrumento estrena `S.arrastre-cola`: conduce el retimado de la cola y afirma que el turno SIGUE en su día; su contraprueba inyecta la mudanza y salta. Test unitario crítico: retimar la cola en su celda da 'retimar', no 'reubicar' (con el bug daba 'reubicar', rojo visto).
**Ley que sale de aquí:** la decisión del arrastre es sobre el TROZO agarrado, no sobre el turno: comparar con `turno.dia` mezcla el ente con su dibujo. Un fallo PREVISTO y evitado también entra a la bitácora: el conocimiento perecedero es "por qué NO se hizo de la forma obvia", que se pierde igual que el de un fallo que sí ocurrió. Caso cruzado (soltar la cola en el día de la cabeza) = reubicar = no-op, sin caso especial: cae de la regla general.
**Traza:** `src/composables/useArrastre.js` (modoArrastre + delta al turno entero + preview re-segmentado); `src/composables/useArrastre.test.js`; `src/components/{FichaTurno,Celda,Barra,Parrilla}.vue` (el trozo agarrado, data-turno); `tools/geometria.check.mjs` (S.suma, S.borde, S.arrastre-cola).

## [2026-07-23] — El día no seguía a la hora: al retimar cruzando medianoche, el turno RETROCEDÍA un día en silencio
**Categoría:** rompe
**Síntoma:** Antonio, ARRASTRANDO, cazó lo que ningún test veía. Arrastra la cabeza de Iker (Jue 16, 22:00→07:00) dentro de su celda; el inicio avanza y cruza medianoche: el rótulo marca 00:45. Suelta, se abre el editor: horas 00:45–09:45, y el selector de Día sigue diciendo JUE 16. Aplica. El turno aparece repartido entre MIÉRCOLES y JUEVES. La hora AVANZÓ y el turno RETROCEDIÓ un día entero. Fallo silencioso de libro: no rompe, no da error, pinta una pantalla perfectamente coherente y perfectamente falsa.
**Qué se probó y DIO VERDE mientras el fallo estaba vivo:** ⭐ TODO. La suite de 50 tests en verde. Los detectores de geometría con sus contrapruebas, todos saltando. `S.suma` verificando que los dos trozos del partido SUMABAN la duración — y sumaban: el turno estaba entero (9 h), solo que en el día equivocado. `S.arrastre-cola` confirmando que retimar la cola NO muda el día — pero solo con retimados pequeños que no cruzan medianoche. Y el editor mostrando "Jue 16" con toda la seguridad del mundo mientras el turno se guardaba en el miércoles. LOS TRES INSTRUMENTOS MEDÍAN INTEGRIDAD, NINGUNO MEDÍA IDENTIDAD.
**Causa raíz:** Un turno guarda su arranque como el par `(dia, inicio)`, donde `dia` es la fecha de calendario en que cae el reloj `inicio`. Cuando una operación mueve el arranque a través de la medianoche, el reloj vuelve a empezar por 00:00 pero el DÍA tiene que sumar ±1. Los TRES caminos que empujan el inicio —arrastre (`retimarTurno`), tirador y teclado del editor (`editarTurno`)— hacían `inicio = formatoHora(min)`, que reduce el reloj mod 24 h CORRECTAMENTE pero se traga el acarreo del día, dejando `dia` quieto. `(Jue, 00:45)` nombra, en tiempo absoluto, un día ENTERO antes que el `(Vie, 00:45)` que se pedía; y como 00:45 < 06:00, `segmentar` lo dibuja en la columna del miércoles. LOS TRES CAMINOS ESTABAN ROTOS IGUAL; solo se vio uno porque solo uno lo arrastró un humano. Si se parchea donde se ve el síntoma (el arrastre), los otros dos siguen rotos esperando a que alguien los pruebe.
**Cómo se cazó:** una PERSONA arrastrando y mirando. Ningún test se le ocurre cruzar medianoche arrastrando y luego comprobar si el día acompañó.
**Arreglo aplicado:** el acarreo del día vive UNA sola vez, en la escritura: `anclarInicio(dia, min)` (fuente única en `useEje.js`) traslada `Math.floor(min/1440)` a la fecha vía `sumarDias`. `retimarTurno` y `editarTurno` pasan los dos por ahí; el `fin` sigue siendo reloj (lo re-deriva `normaliza`). El selector del editor deja de mentir en vivo: un `computed` get/set muestra el día REAL (`sumarDias(dia, floor(iniMin/1440))`), no la base — la invariante "dia = la fecha en que cae el reloj" vale también para lo que se MUESTRA. La red que faltaba nace como test: la PROPIEDAD "una operación que desplaza el tiempo Δ desplaza el arranque ABSOLUTO exactamente Δ, calendario incluido", probada en los TRES caminos (rojos vistos: arrastre `expected -1260 to be 180`; editor/tirador/teclado `expected '2026-07-16' to be '2026-07-17'`). Instrumento: `S.cruce` conduce el arrastre cruzando medianoche y afirma que el cuerpo cae en Vie, nunca retrocede a Mié; contraprueba vía `--selftest`, exit 0. Comprobado que `moverTurno` (reubicar) NO es un cuarto camino roto: solo cambia de celda conservando el reloj, no desplaza el arranque, así que no necesita acarreo.
**Ley que sale de aquí:** ⚠️ LOS INSTRUMENTOS MEDÍAN INTEGRIDAD, NINGUNO MEDÍA IDENTIDAD. Un turno completo en el día que no es pasa cualquier comprobación de integridad (¿entero? ¿los trozos suman?) sin pestañear. Hace falta una prueba de IDENTIDAD: que el instante absoluto tras una operación sea el que se pidió, calendario incluido. Y el corolario: `dia` e `inicio` son UN solo instante, no dos campos independientes; quien mueva uno a través de medianoche mueve el otro. Cuando aparezca la tanda 3 (crear turnos) tendrá que pasar por `anclarInicio` igual.
**Traza:** `src/composables/useEje.js` (sumarDias, anclarInicio); `src/composables/retimarTurno.js` + `editarTurno.js` (acarreo del día); `src/components/EditorTurno.vue` (selector vivo); `src/composables/instante.test.js` (la propiedad, 3 caminos); `src/composables/useEje.test.js` (sumarDias/anclarInicio); `tools/geometria.check.mjs` (S.cruce).

## [2026-07-23] — La barra del editor de un nocturno se dibujaba al PRINCIPIO del eje: anclaba al inicio en vez de a la jornada
**Categoría:** visual
**Síntoma:** Antonio, en el editor de Iker 22:00–07:00: la barra magenta aparecía al PRINCIPIO del eje y las etiquetas iban de 00:00 a 21:00. Un turno que empieza a las 22:00 debería verse cerca del FINAL del eje, no al principio.
**Qué se probó y DIO VERDE mientras el fallo estaba vivo:** ⭐ La suite y el instrumento en verde: ningún detector miraba la POSICIÓN de la barra dentro del eje del editor (el detector del editor medía duración y color, no dónde cae el arranque). Lo cazó el ojo de Antonio.
**Causa raíz:** `ejeEditor` probaba `cabe = iniMin≥E && finMin≤E+1440`. Iker acaba a las 07:00 del día siguiente (finMin 1860 > 1800 = 06:00 siguiente): NO "cabía", y la rama de reserva anclaba la ventana al INICIO (`desde = iniMin = 1320`), plantando las 22:00 al 0 % y las marcas arrancando en 00:00. Un turno que se sale por el final de la jornada (cruza las 06:00 por la cola) es un caso normalísimo, no uno anclado a su propio inicio.
**Cómo se cazó:** ojo humano (Antonio) sobre el editor, con las manos.
**Arreglo aplicado:** `ejeEditor` pasa a `desde = min(iniMin, E)`, `hasta = max(desde+1440, finMin)`: ancla en el arranque del negocio (06:00) cuando el turno empieza dentro, y ALARGA la ventana lo justo para contener la cola que se sale. Iker cae en `[360, 1860]`: 22:00 al 64 % (cerca del final), la cola llegando al 100 %. DEGRADA EXACTO al comportamiento anterior en los dos casos que ya iban bien (turno que cabe / que empieza antes de las 06:00): solo cambia el que estaba roto. Verificado con captura a 1366 y test (`ejeEditor(1320,1860,360)` → `{desde:360,hasta:1860}`, rojo visto con el eje viejo `{desde:1320}`).
**Ley que sale de aquí:** el editor NO usa la ventana fija de la parrilla (que PARTE lo que se sale): su razón de existir es mostrar el turno ENTERO de un vistazo, y recortarle la cola sería una barra recortada, o sea una mentira dibujada. Por eso su ventana se alarga en vez de partir. Son dos frames distintos sobre el mismo dato: parrilla parte, editor contiene.
**Traza:** `src/composables/useEje.js` (ejeEditor); `src/composables/useEje.test.js`.
